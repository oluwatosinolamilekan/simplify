...
<div class="inline-block w-full my-4">
    ...
    @if(isset($button_route))<x-success-anchor href="{{ $button_route }}" id="{{ isset($button_id) ? $button_id : '' }}" class="add_new_button mr-0 normal_letter_spacing float-right">{{ $button_text ?? 'Add New' }}</x-success-anchor>@endif
</div>

<div class="data-table-container bg-white dark:bg-dark-2 p-8 mt-0" id="datatable_{{ $datatable_id }}">
    @if($beforeTableSlot)
        <div class="mt-8">
                ...
                @if($this->searchableColumns()->count())
                    <div class="md:w-64 lg:w-64 flex rounded-lg shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <x-input wire:model.debounce.500ms="search" id="datatable_global_search_{{ $datatable_id }}" class="mt-auto py-2.5 px-9" placeholder="Search..." />
                                ...
                                @elseif($column['type'] === 'checkbox')
                                    <div class="table-cell w-32 overflow-hidden align-top px-6 py-3.5 border-b border-gray-300 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex h-full flex-col items-center space-y-2 focus:outline-none">
                                        ...
                                        <div class="text-center">
                                            <input type="checkbox" id="datatable_toggle_select_all_rows_{{ $datatable_id }}" wire:click="toggleSelectAll" @if(count($selected) === $this->results->total()) checked @endif class="rounded shadow-sm focus:ring focus:ring-opacity-50 form-check-input border mr-2 focus:border-theme-18 focus:ring-offset-theme-18 focus:ring-theme-18" />
                                        </div>
                                    </div>
                                @elseif($column['type'] === 'actions')
                                    <div class="table-cell border-b border-gray-300 w-5 overflow-hidden align-top">
                                        @if ($this->getActiveFiltersProperty())
                                            <x-light-anchor wire:click="clearAllFilters" id="datatable_clear_all_filters_{{ $datatable_id }}" class="mr-2.5 h-8 mt-2.5 pt-1.5 dark:bg-dark-2">
